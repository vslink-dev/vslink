...
- [ ] **Production relay deployment:** a replacement `relay.vslink.dev` Worker has been prepared in the separate server project with Authorization-header authentication. Deploy it, configure its secrets and custom domain, then verify real extension/browser compatibility before publishing. This extension intentionally never sends credentials in the WebSocket URL.
